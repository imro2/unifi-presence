/**
 * J_UnifiPresence.js
 * Configuration interface for UnifiPresence
 *
 * Based on UnifiSensor https://github.com/imro2-github/unifi-sensor
 * and VirtuelSensor https://github.com/toggledbits/VirtualSensor
 */

var UnifiPresence = (function (api) {
	// example of unique identifier for this plugin...
	var uuid = '73955f3a-7775-11e7-b5a5-be2e44b06b34';
	var unifiPresence_svs = 'urn:imro2-github:serviceId:UnifiPresence1';
	var myModule = {};
	...
	function period_set(deviceID, varVal) {
	  api.setDeviceStateVariablePersistent(deviceID, unifiPresence_svs, "Period", varVal, 0);
	} 
	
	function timeout_set(deviceID, varVal) {
	  api.setDeviceStateVariablePersistent(deviceID, unifiPresence_svs, "DeviceTimeout", varVal, 0);
	}

	// functions for additional unifi variables
	
	function url_set(deviceID, varVal) {
	  api.setDeviceStateVariablePersistent(deviceID, unifiPresence_svs, "UnifiURL", varVal, 0);
	}

	function username_set(deviceID, varVal) {
	  api.setDeviceStateVariablePersistent(deviceID, unifiPresence_svs, "UnifiUser", varVal, 0);
	} 
	
	function passwd_set(deviceID, varVal) {
	  api.setDeviceStateVariablePersistent(deviceID, unifiPresence_svs, "UnifiPassword", varVal, 0);
	}
	
	function site_set(deviceID, varVal) {
	  api.setDeviceStateVariablePersistent(deviceID, unifiPresence_svs, "UnifiSite", varVal, 0);
	}
	...
	function UnifiPresenceSettings(deviceID) {
		try {
			init();
			
			var period  = api.getDeviceState(deviceID,  unifiPresence_svs, 'Period');
			var timeout  = api.getDeviceState(deviceID,  unifiPresence_svs, 'DeviceTimeout');
			// variables for additional unifi stuff
			var url  = api.getDeviceState(deviceID,  unifiPresence_svs, 'UnifiURL');
			var username  = api.getDeviceState(deviceID,  unifiPresence_svs, 'UnifiUser');
			var passwd  = api.getDeviceState(deviceID,  unifiPresence_svs, 'UnifiPassword');
			var site  = api.getDeviceState(deviceID,  unifiPresence_svs, 'UnifiSite');
			
			if(isNaN(timeout)) timeout = 0;

			var html =  '<table>' +
				' <tr><td>Poll Period </td><td><input  type="text" id="query_period" size=16 value="' +  period + '" onchange="UnifiPresence.period_set(' + deviceID + ', this.value);"> seconds</td></tr>' +
				' <tr><td>Device Timout </td><td><input type="text" id="query_retries" size=16 value="' +  timeout + '" onchange="UnifiPresence.timout_set(' + deviceID + ', this.value);"> seconds</td></tr>' +
				' <tr><td>Unifi URL </td><td><input type="text" id="unifi_url" size=20 value="' +  url + '" onchange="UnifiPresence.url_set(' + deviceID + ', this.value);"></td></tr>' +
				' <tr><td>Unifi Username </td><td><input type="text" id="unifi_user" size=16 value="' +  username + '" onchange="UnifiPresence.username_set(' + deviceID + ', this.value);"></td></tr>' +
				' <tr><td>Unifi Password </td><td><input type="text" id="unifi_passwd" size=16 value="' +  passwd + '" onchange="UnifiPresence.passwd_set(' + deviceID + ', this.value);"></td></tr>' +
				' <tr><td>Unifi Site </td><td><input type="text" id="unifi_site" size=16 value="' +  site + '" onchange="UnifiPresence.site_set(' + deviceID + ', this.value);"></td></tr>' +
				'</table>';
			html += '<input type="button" value="Save and Reload" onClick="UnifiPresence.ReloadEngine()"/>';
			api.setCpanelContent(html);
		} catch (e) {
            Utils.logError('Error in UnifiPresence.UnifiPresenceSettings(): ' + e);
        }
	}
	...
	function handleRowChange( ev ) {
		...
		/* First the device */
		api.setDeviceStateVariablePersistent( vsdevice, unifiPresence_svs, "MACAddress", macaddress,
		{
			'onSuccess': function() {
				jQuery.ajax({
	...
	function handleAddChildClick( ev ) {
		var el = jQuery( ev.currentTarget );
		var row = el.closest( 'div.row' );
		api.performActionOnDevice( api.getCpanelDeviceId(), unifiPresence_svs, "AddChild", {
		...
		if ( devobj ) {
			...
			while ( true ) {
				...
				if ( txt.match( /^.+$/ ) ) {
					...
					api.performActionOnDevice( devobj.id_parent, unifiPresence_svs, "SetSensorName", { actionArguments: { DeviceNum: dev, NewName: txt } });
					$el.text( txt );
					break;
				}
			}
	...
	function updateCurrentValues() {
		jQuery( 'div#vs-content div.vssensor' ).each( function() {
			...
			var when = parseInt( api.getDeviceStateVariable( vs, unifiPresence_svs, "LastSeen" ) );
			var str;
			if ( ! isNaN( when ) ) {
	...
	function redrawChildren() {
		...
		for ( ix=0; ix<(devices || []).length; ix++ ) {
			...
			if ( v.id_parent == myDevice ) {
				...
				jQuery( 'input#macaddress', col ).val( api.getDeviceStateVariable( v.id, unifiPresence_svs, "MACAddress" ) || "" )
		...
		var enab = 0 !== parseInt( api.getDeviceStateVariable( myDevice, unifiPresence_svs, "Enabled" ) || "0" );
		if ( !enab ) {
			...
			jQuery( 'button#addchild', container ).on( 'click.unifiPresence', handleAddChildClick );
		}

		updateCurrentValues();
		api.registerEventHandler( 'on_ui_deviceStatusChanged', UnifiPresence, 'onUIDeviceStatusChanged' );
	...
	function doVirtualSensors(deviceID) {
		...
		{
			alert(String(e));
			Utils.logError('Error in UnifiPresence.doVirtualSensors(): ' + e);
		}
	}
	...
	myModule = {
		...
		UnifiPresenceSettings : UnifiPresenceSettings,
